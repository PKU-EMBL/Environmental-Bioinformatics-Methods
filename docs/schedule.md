...
<p class="page-lead">十讲课程构成一条从序列资源到群落生态的连续方法链。本页帮助你在每次课前定位问题、课后整理证据，并把阶段性理解逐步汇入课程汇报。</p>

```{admonition} 使用方式
:class: note
课前浏览“核心问题”和课件中的流程图；课中记录关键假设与质量控制；课后用一句话回答“本讲新增了哪一类证据，以及它不能说明什么”。
```
...
| 完成阶段 | 你应该能够回答 |
| --- | --- |
| **L1–L3** | 为什么一个高相似性命中仍不必然等于确定功能？ |
| **L4–L6** | 采样、测序深度与组装策略如何限制后续结论？ |
| **L7–L8** | 一个 MAG 的完整度、污染度和系统发育位置为何需要联合判断？ |
| **L9** | 基因组丰度、功能潜能与转录活性之间有什么证据差异？ |
| **L10** | 多样性变化何时能支持生态机制，何时只能描述模式？ |
